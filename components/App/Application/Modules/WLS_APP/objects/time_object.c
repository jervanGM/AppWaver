...
static int resource_read(anjay_t *anjay,
    ...
    case RID_CURRENT_TIME: {
        ...
        return anjay_ret_i64(ctx, inst->last_notify_timestamp);
    }
...
const anjay_dm_object_def_t **time_object_create(void) {
    ...
    obj->def = &OBJ_DEF;
    char application_names[4][64] = {"Plant start buffer time",
                                     "Plant end buffer time",
                                     "Axis start buffer time",
                                     "Axis end buffer time",
                                    };

    for(int i=0;i<MAX_TIME_INSTANCES;i++)
    {
        time_instance_t *inst = add_instance(obj, i);
...
void time_object_value_update(int64_t *time_data,const anjay_dm_object_def_t *const *def)
{
    ...
    {
        AVS_LIST_FOREACH(it, obj->instances) {
            it->last_notify_timestamp = time_data[it->iid];
        }
    }
}
...
void time_object_update(anjay_t *anjay,const anjay_dm_object_def_t *const *def) 
{
    ...
    AVS_LIST_FOREACH(it, obj->instances) 
    {
        anjay_notify_changed(anjay, obj->def->oid, it->iid, RID_CURRENT_TIME);
    }
}
