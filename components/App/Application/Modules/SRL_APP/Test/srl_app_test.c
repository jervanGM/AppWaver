...
void test_serial_app_check_mayor_faults() {
    ESrlTaskStatus_t fault;
    store_error_in_slot(SERIAL_ERROR_SLOT, -120);
    fault = serial_app_check_faults();
    TEST_ASSERT_EQUAL(fault, SRL_MAYOR_FAULT);
}

void test_serial_app_check_minor_faults() {
    ESrlTaskStatus_t fault;
    store_error_in_slot(SERIAL_ERROR_SLOT, -10);
    fault = serial_app_check_faults();
    TEST_ASSERT_EQUAL(fault, SRL_MINOR_FAULT);
}
...
void test_serial_app_check_no_faults() {
    ...
    store_error_in_slot(SERIAL_ERROR_SLOT, 10);
    fault = serial_app_check_faults();
    TEST_ASSERT_EQUAL(fault, SRL_TASK_OK);
}

void test_serial_app_check_lower_minor_fault_boundary() {
    ESrlTaskStatus_t fault;
    store_error_in_slot(SERIAL_ERROR_SLOT, MINOR_FAULT_THRESHOLD);
    fault = serial_app_check_faults();
    TEST_ASSERT_EQUAL(fault, SRL_TASK_OK);
}
...
void test_serial_app_check_upper_minor_fault_boundary() {
    ESrlTaskStatus_t fault;
    store_error_in_slot(SERIAL_ERROR_SLOT, MAYOR_FAULT_THESHOLD - 1);
    fault = serial_app_check_faults();
    TEST_ASSERT_EQUAL(fault, SRL_MAYOR_FAULT);
}
...
void test_serial_app_check_lower_major_fault_boundary() {
    ESrlTaskStatus_t fault;
    store_error_in_slot(SERIAL_ERROR_SLOT, MAYOR_FAULT_THESHOLD);
    fault = serial_app_check_faults();
    TEST_ASSERT_EQUAL(fault, SRL_TASK_OK);
}
...
void test_serial_app_check_upper_major_fault_boundary() {
    ESrlTaskStatus_t fault;
    store_error_in_slot(SERIAL_ERROR_SLOT, INT8_MAX);
    fault = serial_app_check_faults();
    TEST_ASSERT_EQUAL(fault, SRL_TASK_OK);
}
